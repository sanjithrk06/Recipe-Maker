...
const FieldSettings = ({ type, onTypeChange, saveClicked }) => {
  ...
  return (
    <div className="w-[20%] max-md:w-full flex flex-col justify-between bg-white border border-gray-300 rounded-3xl m-2 p-6">
      <div className="max-md:hidden">
        <div className="mb-4 mt-4 p-2">
          <h2 className=" text-base font-medium pb-2">ADD A FIELD</h2>
          <p className="text-sm font-normal text-gray-600">
            Create new field in the event
          </p>
        ...
        <div className="mb-4 mt-4">
          <span className="text-sm">
            Apply on <strong>all events</strong>
          </span>
          <br />
      ...
      <div className="flex justify-between m-2 mt-5">
        {(type === "ingredients" ||
          ...
          <>
            <button
              onClick={handleBack}
              className="bg-blue-600  text-white p-2 px-4 py-1 pb-[0.4rem] w-auto rounded-xl flex items-center font-medium text-base"
            >
              Back
            </button>
          </>
        ...
        {(type === "spices" || type === "ingredients") && (
          <>
            <button className="bg-transparent  text-white p-2 px-4 py-1 pb-[0.4rem] w-auto rounded-xl flex items-center font-medium text-base"></button>
            <button
              onClick={handleNext}
              disabled={!saveClicked}
              className={`bg-blue-600  text-white p-2 px-4 py-1 pb-[0.4rem] w-auto rounded-xl flex items-center font-medium text-base ${
                saveClicked ? "" : "opacity-50"
              }`}
            >
              Next
            </button>
